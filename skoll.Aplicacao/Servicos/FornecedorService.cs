...
namespace skoll.Aplicacao.Servicos
{
    public class FornecedorService : IFornecedorService
    {
        ...
        public IEnumerable<Fornecedor> GetAll(string search)
        {
            using (var context = _unitOfWork.Create())
            {
                var list = context.Repositorios.FornecedorRepositorio.GetAll();
                if (!string.IsNullOrEmpty(search))
                    return list.Where(e => e.nome.ToUpper().Contains(search.ToUpper()) || e.cpfCnpj.ToUpper().Contains(search.ToUpper()));
                else
                    return list;
            }
        }
